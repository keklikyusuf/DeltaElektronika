...
* Having all related socket comments to be able communicate.
* Datalogger operations class (Basic, Ah and Wh).
* Watchdog operation class
* Battery charging, discharging and cycling operations.
...
2. Datalogging thread for three type of frames
...
6. Cycling thread for following each others for long following operation
...
> It must has socket, threading, time, csv, datetime, sys, logging modules with `importsocket, threading, time, csv, datetime, sys, logging`
...
import logging
```
> After than that, version number and logger has been initialized if debugging is needed.
...
> To be able to use colorfull printing option for better monitoring at therminal page, ColorPrinter class has been created
> Note: colors are limited (purple, blue, cyan, green, yellow, red and normal)
```python
class ColorPrinter:
    purple = '\033[95m'
    blue = '\033[94m'
...
```
> After all related comments has been created, additional functionalities has been added.
> It has shutwdown operation, datalogger operatoin (basic, Ah and Wh), 
> watchdog operation, battery charger, discharger and cycler operations.
```python
...
> As final step testing all functionalities has been added!
...
```
> To be able to run the package itself;
```python
